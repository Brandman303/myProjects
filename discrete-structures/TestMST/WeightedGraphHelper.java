...
public class WeightedGraphHelper<T> extends UnweightedGraphHelper<T> {
    ...
    public MST getMinimumSpanningTree(Vertex<T> startingVertex) {
        ...
        while (T.size() < getSize()) { // While ( size of T < n)

            //
            // algorithm to be completed by student
            // Student Brandon

            WeightedEdge<T> leastCost = null;
            Vertex<T> v = null;

            for (int i = 0; i < T.size(); i++) {
                Vertex<T> vertex = T.get(i);
                for(WeightedEdge<T> edge : neighbors.get(vertex)) {
                    if (!T.contains(edge.v) && (leastCost == null || edge.weight < leastCost.weight)) {
                        leastCost = edge;
                        v = edge.v;
                    }
                }
            ...
            T.add(v);
            parent.put(v, leastCost.u);
            totalWeight += leastCost.weight;

            // End Code by Student
            //

            //I kept getting an error for 106 - 112 so I changed it, I am unsure if I was supposed to change that bit or not. Hope that is ok. - Brandon
    ...
    public class ShortestPathTree extends UnweightedGraphHelper<T>.SearchTree {
        ...
        public ShortestPathTree(Vertex<T> source, Map<Vertex<T>, Vertex<T>> parent, List<Vertex<T>> searchOrder,
                Map<Vertex<T>, Double> cost) {
